...
## Lasso Regression
For this model, we will be running a lasso regression. We will perform a 10 fold cross validation to determine which lambda coefficient minimizes MSE the most and use that to make predictions. 
...
Out of all the models, the downsampled training model that predicted the downsampled testset had the highest AUC.
...
bulding two model on downsampl(balanced) train data and original(imbalanced) train data, then run each model on both orginal(imbalanced) and downsample(balanced) test data
## Logistic Regression
```{r}
# Fit logistic regression model
set.seed(1)
...
```
Next we run cross validation with folds = 5, to find the optimal cp value and further improve the model accuracy. Accuracy rate is 99%. 
```{r}
numFolds <- trainControl(method = "cv", number = 5)

cpGrid <- expand.grid(.cp = seq(0.01, 0.5, 0.01))
train(train$Class~., data = train, method = "rpart", trControl = numFolds, tuneGrid = cpGrid)

trainTreeCV <- rpart(Class~., data=train, method="class", cp= 0.01)
predictionCV <- predict(trainTreeCV, newdata = test, type = "class")
table(test$Class, predictionCV)
```
...
It appears that out of all of the algorithms we performed, Isolation Forest performed the worst. However though, the results are still strong enough to be significant. 